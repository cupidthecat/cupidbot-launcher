module.exports = async function (deps) {
    const { spawn, path, dialog, shell, log, fs, cupidbotDir, ipcMain } = deps;
    ...
    ipcMain.handle(
        ...
        async (event, version, proxy, account, ramPreference) => {
            try {
                const jarPath = path.join(cupidbotDir, `cupidbot-${version}.jar`);
    ...
    ipcMain.handle(
        ...
        async (event, version, proxy, ramPreference) => {
            ...
            if (
                fs.existsSync(
                    path.join(cupidbotDir, 'non-jagex-preferred-profile.json')
                )
            ) {
                try {
            ...
            if (isInstalled) {
                ...
                dialog
                    .showMessageBox({
                        ...
                        message:
                            'Java Development Kit (JDK) 17 is required to run CupidBot. Install it locally, then reopen the launcher.',
                        detail:
                            'CupidBot Launcher does not open external download pages. On Linux, install an OpenJDK 17 package from your system package manager. On Windows or macOS, install JDK 17 locally and make java available on PATH.',
                        buttons: ['OK']
                    })
                    .then(() => {
                        log.info('Java is not installed or not available on PATH.');
                    });
            }
        });
    }
