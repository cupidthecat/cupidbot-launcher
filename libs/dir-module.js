...
const cupidbotDir = path.join(homeDir, '.cupidbot');
...
/**
 * Check for the existence of the clients_jar_ttl.json file in the cupidbot directory.
...
async function getClientsJarTTL() {
    ...
    const filePath = path.join(cupidbotDir, 'clients_jar_ttl.json');
    try {
        await fs.mkdir(cupidbotDir, { recursive: true });
        try {
...
async function cleanupUnusedClientsJar(latestVersion) {
    ...
    let updated = false;
    try {
        const files = await fs.readdir(cupidbotDir);
        const jarFiles = files.filter(
            (f) =>
                f.endsWith('.jar') &&
                f.startsWith('cupidbot-') &&
                !f.includes('launcher')
        );
        const now = Date.now();
        for (const jarFile of jarFiles) {
            let version = jarFile.replace('.jar', '');
            version = version.replace('cupidbot-', '');
